...
# and `pip install demo_e2e` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "demo_e2e", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n demo_e2e -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "demo_e2e", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "demo_e2e", "YOUR_CONSOLE_SCRIPT"]
